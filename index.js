...
io.on('connection', (socket) => {
    ...
    socket.on("user-message", (message) => { // this is the mesage u send while connecting 
        console.log(message);
        // Emit the response message after a delay
        setTimeout(() => {
            io.emit('response-message', JSON.stringify({ title: 'Drawing stoped', body: message })); 
        }, 9000); // change this to set the time delay 
    });
});
